...
TMP_OUT = Path.cwd() / "docs" / "_tmp_毕业设计正文模板_安全目录40.docx"
...
LINES = [
    ...
    ("第7章 总结与展望", "29", 0), ("参考文献", "30", 0), ("致谢", "31", 0), ("附录A 系统主要接口", "32", 0), ("附录B 核心代码片段与接口说明", "33", 0), ("附录C 系统部署与运行说明", "35", 0), ("附录D 接口返回字段示例", "36", 0),
]


def set_font(run, size=9.2):
    run.font.name = "Times New Roman"
    run._element.rPr.rFonts.set(qn("w:eastAsia"), "宋体")
    run.font.size = Pt(size)


def format_line(p, level):
...
def main():
    ...
    anchor_idx = next(i for i in range(toc_idx + 1, len(doc.paragraphs)) if doc.paragraphs[i].text.strip().replace(" ", "") == "第1章绪论")
    anchor = doc.paragraphs[anchor_idx]

    for p in list(doc.paragraphs[toc_idx + 1:anchor_idx]):
        delete_paragraph(p)

    inserted = []
    for title, page, level in LINES:
